...
to train
  ...
  repeat examples-per-epoch [
    ask input-nodes [ set activation random-float 1 ]
    propagate
    backpropagate
  ]
...
to backpropagate
  ...
  ask input-node-1 [print activation]
...
to-report reLu [input]
  report ifelse-value input <= 0 [ 0 ] [ input ]
end
...
;; the function below was made initially to be used the same way as the original old function was, to test the model on some data and return a value.
;; but it then discarded because we needed multiple values to be returned, which is apparently can't be done in NetLogo? or we didn't look hard enough, but overall we inserted this function into the
;; function above, so the one below is no further needed, but will be kept anyways.

to-report result-for-inputs [n1 n2 n3 n4 n5]
  ...
  ask input-node-5 [ set activation n5 ]
